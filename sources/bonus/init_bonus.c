...
/*   Updated: 2023/11/19 16:50:13 by roglopes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	data_init(t_fractal *fractal)
{
	...
	fractal->iterations_defintion = 1100;
	fractal->axle_x = 0.0;
	fractal->axle_y = 0.0;
	fractal->zoom = 1.0;
}
...
void	ft_movement_handler(void *param)
{
	...
	if (mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_ESCAPE))
		mlx_close_window(fractal->mlx_main_screen);
			...
			mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_A))
		fractal->axle_x += (0.5 * fractal->zoom);
	else if (mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_RIGHT) || \
			mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_D))
		fractal->axle_x -= (0.5 * fractal->zoom);
	else if (mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_UP) || \
			mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_W))
		fractal->axle_y -= (0.5 * fractal->zoom);
	else if (mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_DOWN) || \
			mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_S))
		fractal->axle_y += (0.5 * fractal->zoom);
	else if (mlx_is_key_down(fractal->mlx_main_screen, MLX_KEY_C))
		fractal->iterations_defintion += 10.42;
